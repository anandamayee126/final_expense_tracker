...
window.addEventListener('load', async (req, res) => {
    ...
    const all_expense = await axios_1.default.get('http://localhost:4000/user/getExpense', { headers: { 'Authorization': token } });
...
function addDailyExpense(e) {
    ...
    const token = localStorage.getItem('token');
    axios_1.default.post('http://localhost:4000/user/dailyExpense', expense, { headers: { 'Authorization': token } }).then(response => { console.log(response); }).catch(err => { console.log(err); });
}
function displayExpense(expense) {
    ...
    const token = localStorage.getItem('token');
    button.onclick = () => {
        axios_1.default.delete('http://localhost:4000/user/delete/' + expense.id, { headers: { 'Authorization': token } })
            .then((res) => {
            if (res.status == 200)
                ul.removeChild(li);
        })
...
async function showLeaderboard() {
    const token = localStorage.getItem('token');
    console.log(token);
    const userLeaderboard = await axios_1.default.get('http://localhost:4000/premium/showLeaderboard', { headers: { "Authorization": token } });
...
razor_pay.onclick = async function (e) {
    const token = localStorage.getItem('token');
    const response = await axios_1.default.get('http://localhost:4000/user/premiumMembership', { headers: { "Authorization": token } });
    console.log("response", response);
    var options = {
        "key": response.data.key_id,
        "order_id": response.data.order.id,
        "handler": async function (response) {
            const result = await axios_1.default.post('http://localhost:4000/user/updateTransaction', {
    ...
    const rzp1 = new razorpay_1.default(options);
    rzp1.open();
    e.preventDefault();
